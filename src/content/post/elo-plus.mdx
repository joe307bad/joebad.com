...
**The Elo+ model analyzed all 2,430 MLB games from 2024 and achieved a 2.8% improvement in predictive accuracy over the baseline <span id="2-back-to-top">[^[2]^](#2)</span> using a [70/15/15 train-test split validation methodology](https://learn.microsoft.com/en-us/dotnet/machine-learning/how-to-guides/prepare-data-ml-net#split-data-into-training--test-sets).**
...
$$
\text{Baseline} = \frac{\max(\text{HomeWins}, \text{AwayWins})}{\text{TotalGames}}
$$

This is our reality check - the simplest possible prediction strategy! If home teams won 55% of games in our dataset, the baseline is 55% (always pick home). If away teams won more, we'd always pick away. Any sophisticated prediction system must beat this naive approach to be worth the complexity. For Elo+, beating baseline by 2.8% means our machine learning enhancements are genuinely adding value beyond just "pick the more common outcome." **This baseline calculation is implemented in Elo+ as a simple majority-class predictor - we just count which outcome happened more often in our dataset.**
...
<span id="2">2.</span> The baseline is the simplest possible prediction strategy. For Elo+, the baseline is always picking whichever outcome (home wins or away wins) occurred more frequently in the dataset - essentially the "majority class" approach. If the ML model beats this baseline by 2.8%, it means the model's accuracy is 2.8 percentage points higher than this naive approach. This shows the model has learned meaningful patterns beyond just picking the most common outcome. <a href="#2-back-to-top">^</a>