...
function parseRSS(xmlText: string): RSSData {
  ...
  try {
    ...
    const items: RSSItem[] = itemsArray.map((item: any) => {
      ...
      const itemDescription = item.description || item.summary || item.content || '';
      ...
      if (typeof itemDescription === 'string') {
        ...
        cleanDescription = itemDescription['#text'] || itemDescription.toString();
      }
...
async function processReactPage(filePath: string, rssData?: RSSData): Promise<PageData> {
  console.log(`🔄 Compiling React component: ${filePath}`);
  
  // Create a temporary JS file for compilation
  const relativePath = path.relative(process.cwd(), filePath);
  const tempDir = 'temp';
  const tempFile = path.join(tempDir, path.basename(filePath, path.extname(filePath)) + '.js');
  ...
  try {
    ...
    if (!Component || typeof Component !== 'function') {
      throw new Error(`No default export found or export is not a React component in ${filePath}`);
    }
    ...
    const title = metadata.title || Component.displayName || path.basename(filePath, path.extname(filePath));
...
async function buildTailwind(): Promise<string> {
  ...
  try {
    ...
    // Build Tailwind CSS
    await execAsync('npx tailwindcss -i src/styles/main.css -o dist/assets/main.css --minify');
...
function createHtmlTemplate(title: string, content: string, css: string, pageType: 'markdown' | 'react' = 'markdown'): string {
  // For React pages, don't wrap in prose classes as they likely have their own styling
  const contentWrapper = pageType === 'react' ? content : `
  ...
  return `<!DOCTYPE html>
<html lang="en" class="h-full w-full p-2 bg-[#F8F2F1]">
<head>
  <meta charset="UTF-8">
...
async function build() {
  console.log('🏗️  Building static site with Tailwind, React support, and RSS feeds...');
  ...
  for (const pageData of pages) {
    ...
    if (pageData.type === 'markdown' && Object.keys(pageData.frontmatter).length > 0) {
      const metaHtml = `
    ...
    const html = createHtmlTemplate(pageData.title, contentWithMeta, css, pageData.type);
  ...
  console.log(`📊 Generated ${pages.length} pages (${pages.filter(p => p.type === 'markdown').length} Markdown, ${pages.filter(p => p.type === 'react').length} React) with Tailwind CSS and RSS data`);
}
...
export { build, processMarkdown, processReactPage, createHtmlTemplate, fetchRSSFeed };